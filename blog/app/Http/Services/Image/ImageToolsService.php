...
class ImageToolsService
{
    ...
    public function setCurrnetImageName()
    {
        return !empty($this->image) ? $this->setImageName(pathinfo($this->image->getClientOriginalName(), PATHINFO_FILENAME)) : false;

        //$_FILES['image']['name'] == pathinfo($this->image->getClientOriginalName(),PATHINFO_FILENAME)
    }
    ...
    protected function provider()
    {
        // set properties
            ...
            $this->getImageDirectory() ?? $this->setImageDirectory(date('Y') . DIRECTORY_SEPARATOR .
            date('m') . DIRECTORY_SEPARATOR . date('d'));
